...
export class AllDepartmentsComponent implements OnInit, OnDestroy {
  ...
  exportExcel() {
    const exportData = this.dataSource.filteredData.map((x) => ({
      ...
      Phone: x.phone,
      Email: x.email,
      'Student Capacity': x.student_capacity,
      'Established Year': x.establishedYear,
      'Total Faculty': x.totalFaculty,
    }));

    TableExportUtil.exportToExcel(exportData, 'department_export');
  }
