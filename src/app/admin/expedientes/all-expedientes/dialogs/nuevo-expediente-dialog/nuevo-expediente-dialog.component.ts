...
@Component({
  ...
  templateUrl: './nuevo-expediente-dialog.component.html',
  styleUrls: ['./nuevo-expediente-dialog.component.scss'],
  imports: [
...
export class NuevoExpedienteDialogComponent implements OnInit, OnDestroy {
  searchTerm  = '';
  ...
  constructor(
    public dialogRef: MatDialogRef<NuevoExpedienteDialogComponent>,
  ...
  selectStudent(student: StudentItem) {
    ...
    this.svc.getOrCreate(student.id).subscribe({
      next: (exp: Expediente) => {
        this.creating = false;
        // Enrich the expediente with student data for the table
        const enriched: Expediente = {
          ...exp,
          studentId: {
            _id:    student.id,
            name:   student.name,
      ...
      error: (err) => {
        this.creating = false;
        this.snackBar.open(err.message || 'Error al crear el expediente', '', {
