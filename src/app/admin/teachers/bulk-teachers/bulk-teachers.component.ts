...
@Component({
  selector: 'app-bulk-teachers',
  templateUrl: './bulk-teachers.component.html',
  imports: [
    CommonModule,
    RouterLink,
    BreadcrumbComponent,
...
export class BulkTeachersComponent {
  breadscrums = [{ title: 'Importar Docentes', items: ['Admin', 'Docentes'], active: 'Importar masivo' }];
  ...
  displayedColumns = ['status', 'name', 'dni', 'usernamePreview', 'email', 'mobile', 'gender'];
  ...
  constructor(private svc: TeachersService, private snackBar: MatSnackBar) {}
  ...
  private parseSheet(rows: any[][]) {
    ...
    for (const cols of dataRows) {
      ...
      if (!name)        { status = 'dni-error'; statusMsg = 'Nombre vacío'; }
        ...
        status = 'dni-error';
        statusMsg = `Cédula inválida (${digits.length} dígitos, se requieren 10)`;
      }

      parsed.push({
        name: name || '(sin nombre)',
        dni: digits || rawDni,
        usernamePreview: name ? this.previewUsername(name) : '—',
    ...
    if (!parsed.length) { this.parseError = 'No se encontraron filas con datos.'; return; }
  ...
  private checkDbDuplicates() {
    ...
    this.svc.checkBulkDuplicates(dnis, emails).subscribe({
      next: (res) => {
  ...
  /** Genera preview indicativo del username (el backend resuelve conflictos reales) */
  private previewUsername(name: string): string {
  ...
  downloadTemplate() {
    const ws = XLSX.utils.aoa_to_sheet([
      ['nombre', 'cedula', 'email', 'celular', 'genero'],
      ['BOLAÑOS SALTOS ANGEL ABDON', '1234567890', 'angel@mail.com', '0991234567', 'Masculino'],
    ]);
    ws['!cols'] = [{ wch: 30 }, { wch: 14 }, { wch: 25 }, { wch: 14 }, { wch: 12 }];
  ...
  submit() {
    ...
    this.svc.bulkCreate(payload).subscribe({
      next: (res) => {
        this.result = res;
        this.isSubmitting = false;
        const msg = res.failureCount === 0
          ? `${res.successCount} docentes registrados correctamente`
