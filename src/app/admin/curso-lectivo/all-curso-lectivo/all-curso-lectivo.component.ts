...
import { CursoLectivoFormComponent } from './dialogs/form-dialog/form-dialog.component';
import { CursoLectivoDeleteComponent } from './dialogs/delete/delete.component';
...
@Component({
  selector: 'app-all-curso-lectivo',
  templateUrl: './all-curso-lectivo.component.html',
  styleUrls: ['./all-curso-lectivo.component.scss'],
...
export class AllCursoLectivoComponent implements OnInit, OnDestroy {
  columnDefinitions = [
    { def: 'select',        label: 'Checkbox',      type: 'check',     visible: true },
    { def: 'cursoDisplay',  label: 'Curso',         type: 'text',      visible: true },
    { def: 'academicYear',  label: 'Año Lectivo',   type: 'text',      visible: true },
    { def: 'tutorName',     label: 'Tutor',         type: 'text',      visible: true },
    { def: 'inspectorName', label: 'Inspector',     type: 'text',      visible: true },
    { def: 'psicologoName', label: 'Psicólogo',     type: 'text',      visible: true },
  ...
  selection = new SelectionModel<CursoLectivo>(true, []);
  ...
  breadscrums = [{ title: 'Cursos Lectivos', items: ['Oferta Educativa'], active: 'Cursos Lectivos' }];
  ...
  editCall(row: CursoLectivo) { this.openDialog('edit', row); }

  openDialog(action: 'add' | 'edit', data?: CursoLectivo) {
    const varDirection: Direction = localStorage.getItem('isRtl') === 'true' ? 'rtl' : 'ltr';
    const dialogRef = this.dialog.open(CursoLectivoFormComponent, {
      width: '60vw', maxWidth: '100vw',
      data: { cursoLectivo: data, action },
  ...
  private updateRecord(updated: CursoLectivo) {
  ...
  deleteItem(row: CursoLectivo) {
    const dialogRef = this.dialog.open(CursoLectivoDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  exportExcel() {
    const exportData = this.dataSource.filteredData.map(x => ({
      ...
      Tutor: x.tutorName, Inspector: x.inspectorName,
      Psicólogo: x.psicologoName, Estado: x.status,
    }));
    TableExportUtil.exportToExcel(exportData, 'curso_lectivo_export');
  }
  ...
  onContextMenu(event: MouseEvent, item: CursoLectivo) {
