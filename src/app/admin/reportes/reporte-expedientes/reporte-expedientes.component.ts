...
import { TIPO_OPTIONS } from '../../expedientes/all-expedientes/expediente.model';
...
export class ReporteExpedientesComponent implements OnInit, OnDestroy {
  breadscrums = [{ title: 'Reporte Expedientes', items: ['Reportes'], active: 'Expedientes Disciplinarios' }];
  ...
  generar() {
    this.loading = true;
    this.http.get<any>(`${environment.apiUrl}/expedientes/reporte`).subscribe({
      next: (res) => {
        this.allData = (res.data ?? []).map((r: any) => ({
          studentName:          r.studentName          ?? r.expediente?.studentId?.name ?? '—',
          studentDni:           r.studentDni           ?? r.expediente?.studentId?.dni  ?? '—',
  ...
  exportXls() {
    ...
    XLSX.writeFile(wb, `reporte-expedientes-${new Date().toISOString().slice(0, 10)}.xlsx`);
  }
